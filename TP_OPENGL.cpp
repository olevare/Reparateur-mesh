...
Camera *camera = new CameraFreeFly(0.1,0.1,0.5,0,0,0);
...
{  
  ...
  glutCreateWindow("Projet Médical");
...
void Projet()
{
  ...
  if(load == 0)
  {
    mesh.loadOffFile("OFF_Model/trice2.off");
    ...
    /*for(double i = mesh.minVertex.getX(); i < mesh.maxVertex.getX(); i += resolution)
    {
      for(double j = mesh.minVertex.getY(); j < mesh.maxVertex.getY(); j += resolution)
      {
        for(double k = mesh.minVertex.getZ(); k < mesh.maxVertex.getZ(); k += resolution)
        {

          Point p = Point((double)(i), (double)(j), (double)(k));
          Octree o = Octree(p, resolution);

          float boxcenter[3] = {i + (resolution / 2), j + (resolution / 2), k + (resolution / 2)};
          float boxhalfsize[3] = {resolution / 2, resolution / 2, resolution / 2};

          for(int caca = 0; caca < mesh.nbTriangle * 3; caca += 3)
          {
            float triangles[3][3];

            triangles[0][0] = mesh.pointsArray[(mesh.indiceArray[caca]) * 3];
            triangles[0][1] = mesh.pointsArray[(mesh.indiceArray[caca]) * 3 + 1];
            triangles[0][2] = mesh.pointsArray[(mesh.indiceArray[caca]) * 3 + 2];

            triangles[1][0] = mesh.pointsArray[(mesh.indiceArray[caca + 1]) * 3];
            triangles[1][1] = mesh.pointsArray[(mesh.indiceArray[caca + 1]) * 3 + 1];
            triangles[1][2] = mesh.pointsArray[(mesh.indiceArray[caca + 1]) * 3 + 2];

            triangles[2][0] = mesh.pointsArray[(mesh.indiceArray[caca + 2]) * 3];
            triangles[2][1] = mesh.pointsArray[(mesh.indiceArray[caca + 2]) * 3 + 1];
            triangles[2][2] = mesh.pointsArray[(mesh.indiceArray[caca + 2]) * 3 + 2];

            if(triBoxOverlap(boxcenter, boxhalfsize, triangles))
            {
              v.push_back(o);
              break;
            }
          ...
          //affiche le maillage dans la grille
          //o.DrawVoxel();
  ...
  /*for(int i = 0; i < v.size(); i++)
  {
    ...
    glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);//LINE = polygone plein
    glCullFace(GL_FRONT);
    mesh.drawMesh();

    glPolygonMode(GL_FRONT_AND_BACK, GL_LINE);//FILL = polygone vide
    glCullFace(GL_BACK);
    mesh.drawMesh();
}
